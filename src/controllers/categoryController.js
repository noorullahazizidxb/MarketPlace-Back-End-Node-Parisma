...
export const categoryController = {
  ...
  const full = await prisma.category.findUnique({ where: { id: cat.id }, include: { children: true } });
  res.apiSuccess(full, 'Created', 201);
  },

  async list(req, res) {
    // Return all categories without pagination
  const cats = await prisma.category.findMany({ include: { children: true }, orderBy: { createdAt: 'desc' } });
...
categoryController.update = async function (req, res) {
  ...
  const full = await prisma.category.findUnique({ where: { id }, include: { children: true } });
